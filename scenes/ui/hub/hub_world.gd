...
## Renders 3D hangar with unshaded Y-billboard character cutouts and Psycho-Pop UI.
...
const PILOT_ROSTER: Array[Dictionary] = [
	{
		...
		"pedestal_pos": Vector3(-3.75, 0.15, 0.3)
	},
	{
		"id": &"valentina",
		"name": "Valentina",
		...
		"pedestal_pos": Vector3(-2.25, 0.15, 0.0)
	},
	{
		"id": &"kira",
		"name": "Kira",
		...
		"pedestal_pos": Vector3(-0.75, 0.15, -0.2)
	},
	{
		"id": &"selene",
		"name": "Selene",
...
func _setup_camera() -> void:
	if camera:
		camera.fov = 85.0
		camera.position = Vector3(0.0, 1.8, 4.2)
...
func _apply_psychopop_styles() -> void:
	...
	if btn_desplegar:
		var sb_desp := StyleBoxFlat.new()
		sb_desp.bg_color = COLOR_HOT_PINK
		sb_desp.set_corner_radius_all(0)
		sb_desp.border_width_left = 4
		sb_desp.border_width_top = 2
		sb_desp.border_width_right = 4
		sb_desp.border_width_bottom = 4
		sb_desp.border_color = COLOR_PURE_WHITE
		sb_desp.set_content_margin_all(14.0)
		btn_desplegar.add_theme_stylebox_override("normal", sb_desp)
		btn_desplegar.add_theme_stylebox_override("hover", sb_desp)
		btn_desplegar.add_theme_stylebox_override("pressed", sb_desp)
		btn_desplegar.add_theme_color_override("font_color", COLOR_DEEP_BLACK)
		btn_desplegar.add_theme_color_override("font_hover_color", COLOR_PURE_WHITE)
		btn_desplegar.add_theme_font_size_override("font_size", 22)
		_setup_button_elastic_tween(btn_desplegar)
	
	if btn_volver:
		var sb_vol := create_psychopop_stylebox(COLOR_DEEP_BLACK, COLOR_HOT_PINK, 4, 2, 4, 2)
		btn_volver.add_theme_stylebox_override("normal", sb_vol)
		btn_volver.add_theme_stylebox_override("hover", sb_vol)
		btn_volver.add_theme_stylebox_override("pressed", sb_vol)
		btn_volver.add_theme_color_override("font_color", COLOR_PURE_WHITE)
		btn_volver.add_theme_font_size_override("font_size", 18)
		_setup_button_elastic_tween(btn_volver)
...
func _build_pilot_selector_buttons() -> void:
	...
	for i in range(PILOT_ROSTER.size()):
		...
		var sb_btn := create_psychopop_stylebox(COLOR_DEEP_BLACK, COLOR_HOT_PINK, 3, 1, 1, 1)
		sb_btn.set_content_margin_all(6.0)
		...
		btn.add_theme_color_override("font_color", COLOR_PURE_WHITE)
...
func _select_pilot(index: int, animate_card: bool = true) -> void:
	...
	# Kill active tweens from previous selections to avoid property fighting
	for tw in _pilot_tweens:
		if is_instance_valid(tw) and tw.is_running():
			tw.kill()
	_pilot_tweens.clear()
	
	# Highlight selected 3D sprite and reset others
	for i in range(sprite_nodes.size()):
		...
		if i == index:
			tw.tween_property(sp, "scale", Vector3(1.15, 1.15, 1.15), 0.35)
			sp.modulate = Color(1.2, 1.2, 1.2, 1.0)
		else:
			tw.tween_property(sp, "scale", Vector3.ONE, 0.25)
			sp.modulate = Color(0.75, 0.75, 0.85, 0.9)
