...
func _display_current_navigator(animate: bool = true, slide_direction: int = 0) -> void:
	...
	# Marco central resplandeciente según color de la navegante
	if artwork_frame:
		var af_sb := artwork_frame.get_theme_stylebox("panel")
		if af_sb is StyleBoxFlat:
			var af_dup := af_sb.duplicate() as StyleBoxFlat
			if is_unlocked:
				af_dup.border_color = nav_data.theme_color
				af_dup.shadow_color = Color(nav_data.theme_color.r, nav_data.theme_color.g, nav_data.theme_color.b, 0.3)
			else:
				af_dup.border_color = Color(0.3, 0.35, 0.45, 0.7)
				af_dup.shadow_color = Color(0, 0, 0, 0)
			artwork_frame.add_theme_stylebox_override("panel", af_dup)

	# 6. Cita de Radio / Comms
	if radio_dialogue:
	...
	# 8. Arte Full-Body Central & Bloqueo
	if fullbody_texture:
